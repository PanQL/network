...
{    
	...
	if(nlen==1)    
	{        
		*(unsigned char*)&answer = *(unsigned char *)iphd1;        
		sum += answer;    
	}    	
	sum -= checksum;    
...
void *thr_fn(void *arg)
{
	...
	int check_sum_count=0;
	int iochange = 0;
	
	ip_recvpkt = (struct ip*)malloc(sizeof(struct ip));
	if((recvfd=socket(AF_PACKET,SOCK_DGRAM,htons(ETH_P_ALL)))==-1)
	{
		printf("recvfd socket error !!\n");
	}
	...
	while(1)
	{
		...
		if(recvlen>0)
		{
			ip_recvpkt = (struct ip *)skbuf;
			//这个源ip地址是测试服务器的ip地址
			if(ip_recvpkt->ip_src.s_addr == inet_addr("192.168.1.1")  &&((ip_recvpkt->ip_ttl != 64) &&(ip_recvpkt->ip_ttl != 128)) && iochange!=0)
			{
				printf("\n");
				printf("收到的报文:\n");
				printf("Source IP is %s\n",inet_ntoa(ip_recvpkt->ip_src));
				printf("Destination IP is %s\n",inet_ntoa(ip_recvpkt->ip_dst));
				if((ip_recvpkt->ip_ttl != 63) &&(ip_recvpkt->ip_ttl != 127))
				{
					printf("TTL is error!!\n");
				}
				printf("TTL is %d\n",ip_recvpkt->ip_ttl);
				printf("checksum is %x\n",ip_recvpkt->ip_sum);
				check_sum_count = check_sum(ip_recvpkt,sizeof(struct ip),ip_recvpkt->ip_sum);
			...
			if(ip_recvpkt->ip_src.s_addr == inet_addr("192.168.1.1") &&((ip_recvpkt->ip_ttl == 64) ||(ip_recvpkt->ip_ttl == 128)))
			{
				if(iochange >= 5)
				{
					printf("没有收到包!!!!\n");
