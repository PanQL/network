...
netlink_route_multipath (int cmd, struct prefix *p, struct rib *rib,
                         int family)
{
	//add by wsd for selfroute
 if(rib->type == 3 || rib->type == 4)
 {
	  int sendfd;
	  int sendlen=0;
	  int tcpcount=0;
	  ...
	  selfrt =&buf;
	  selfrt->selfprefixlen = p->prefixlen;
	  selfrt->selfprefix	= p->u.prefix4;
	  selfrt->selfifindex	= rib->nexthop->ifindex;
	  selfrt->selfnexthop	= rib->nexthop->gate.ipv4;
	  selfrt->cmdnum        = cmd;
